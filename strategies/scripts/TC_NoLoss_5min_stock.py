...
def resolve_option_symbol(option_type, spot_ltp):
    ...
    Lotsize is fetched from the API response (master contract DB).
    Falls back to DEFAULT_LOT_SIZE only if the API doesn't include it.
    """
    url = f"{HOST}/api/v1/optionsymbol"
    expiry_date = STOCK_MONTHLY_EXPIRY
    payload = {
        "apikey": API_KEY,
        "underlying": SYMBOL,
