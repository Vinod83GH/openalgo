...
class PnLMonitor(threading.Thread):
    ...
    def _get_auth_token_for_broker(self, broker_name: str):
        try:
            ...
            if auth_obj is None:
                logger.debug(f"PnL monitor: no active session for broker '{broker_name}', skipping.")
                return None
            return decrypt_token(auth_obj.auth)
        except Exception as e:
            logger.debug(f"PnL monitor: could not retrieve auth token for broker '{broker_name}': {e}")
            return None
    ...
    def _poll_all_active_brokers(self) -> None:
        ...
        update_kill_switch_pnl = _ks_db.update_kill_switch_pnl
        ...
        for config in configs:
            ...
            try:
                ...
                # 4. Cache P&L in DB so status endpoint can read it without a live call
                try:
                    update_kill_switch_pnl(broker_name, current_pnl)
                except Exception as e:
                    logger.debug(f"PnL monitor: could not cache P&L for '{broker_name}': {e}")

                # 5. Evaluate thresholds — activates kill switch if breached
                ...
                logger.warning(f"PnL monitor: error processing broker '{broker_name}': {e}")
